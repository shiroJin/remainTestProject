...
- (void)layoutSubviews
{
    ...
    CGFloat y = self.mainView.sd_height - size.height - 12;
...
- (UICollectionViewCell *)collectionView:(UICollectionView *)collectionView cellForItemAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    if (!cell.hasConfigured) {
        ...
        cell.imageView.contentMode = self.bannerImageViewContentMode;
         [cell addGestureRecognizer:[[UITapGestureRecognizer alloc] initWithTarget:self action:@selector(actionTapCell:)]];
    }
    
    cell.showBottomBg = _showBottomBg;

    return cell;
}

///modify by wjy  修复在图片为1个的时候 drag也会引发didSelect的问题
- (void)actionTapCell:(UITapGestureRecognizer *)tap {
    
    int itemIndex = (self.mainView.contentOffset.x + self.mainView.sd_width * 0.5) / self.mainView.sd_width;
    int index = itemIndex % self.imageURLsGroup.count;
